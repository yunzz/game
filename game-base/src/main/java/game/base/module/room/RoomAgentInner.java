 ...
 */
@AllLife
class RoomAgentInner implements LifeCycle, RoomAgent {

    private final EventExecutor executor;
    private BlockingQueue<RoomMsg> messageQueue;
    ...
    public RoomAgentInner(EventExecutor executor, BlockingQueue queue) {
    ...
    public boolean sendMessage(RoomMsg o) {
        ...
        if (o instanceof MsgTimerTask) {

            MsgTimerTask o1 = (MsgTimerTask) o;
            timer.newTimeout(o1.getTask(), o1.getDelay(), o1.getUnit());
            return true;
        }
    ...
    class RoomTask implements Runnable {
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    RoomMsg msg = messageQueue.poll(pollTimeout, TimeUnit.MILLISECONDS);
                    if (msg == null) {
                        continue;
                    }
        ...
        private Throwable processMsg(RoomMsg msg) {
            ...
            if (msg instanceof MsgRoomCreateInner) {
                ...
                if (room != null) {
                    cause = msg(room, msg.msg(), ((MsgAsyncInner<Object>) msg).getComplete());
                }
            } else {
                MsgRoomInner roomInner = (MsgRoomInner) msg;
                Room room = rooms.get(roomInner.roomId);
                if (room != null) {
